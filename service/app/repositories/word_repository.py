...
class WordRepository:
    ...
    # --- create ---
    def create(self, word: WordModel) -> PyObjectId: 
        """ insert a new word item """
        doc = word.model_dump(by_alias=True, exclude_none=True)
        res = self.col.insert_one(doc)
        return res.inserted_id

    # --- read ---
    def find_by_id(self, id: PyObjectId) -> PyObjectId | None: 
        doc = self.col.find_one({"_id": id})
        return doc["_id"] if doc else None

    def find_item_by_word_ids(self, word_ids: List[PyObjectId]) -> List[WordModel]:
    ...
    def search_items_by_word_subseq(
        self,
        subseq_pattern: str,
        max_num: int,
